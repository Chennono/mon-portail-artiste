...
(`https://…/p/<id>`). Il permet le bouton **« Publier (lien avec photos) »** de l'app.

Les pages **expirent automatiquement après 90 jours** (minimisation des données / RGPD).
...
Puis re-commitez / re-déployez le site (GitHub Pages). Le bouton **« Publier »**
...
- **RGPD** : les pages publiées sont stockées sur le serveur (contrairement au reste de
  l'app, 100 % local). Prévenez les artistes, et la durée de rétention (90 j) est réglable
  via `EXPIRATION_DAYS` dans `worker.js`.
