...
- partage simplifie par un seul bouton « Copier le lien de ma page » : la page et ses photos sont publiees par un petit service Cloudflare Worker, puis le vrai lien `https://.../p/<id>` est automatiquement copie. Le bouton n'apparait que si `PUBLISH_ENDPOINT` est renseigne dans `app.js`. Code et instructions de deploiement dans `worker/` (Worker + KV, pages expirant a 90 jours);
...
- module "Partager mon portail" apres generation, volontairement reduit a un seul bouton de publication et de copie du lien;
