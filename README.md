...
- publication par le bouton « Publier ma page » : la page et ses photos sont mises en ligne par un petit service Cloudflare Worker. La premiere publication cree une adresse personnalisee et permanente (`/p/nom-artiste`) ainsi qu'un jeton d'edition conserve localement ; les mises a jour suivantes remplacent la page a la meme adresse. Le lien est automatiquement copie. Le bouton n'apparait que si `PUBLISH_ENDPOINT` est renseigne dans `app.js`;
- bouton « Depublier » : retire la page en ligne via le jeton d'edition (suppression cote serveur, RGPD);
- import de CV (« Importer un CV ») : lecture du texte d'un fichier PDF, Word (.docx) ou texte directement dans le navigateur (PDF.js / mammoth charges a la demande), ajout a la conversation, puis generation automatique de la page (ideal pour les artistes ayant deja un CV);
- style de chaque photo directement dans l'apercu (photo de profil, visuel principal et photos d'oeuvres) : cadrage (remplir/entier), format (auto/carre/portrait/paysage/large), coins (droits/arrondis/cercle) et filtre (N&B, sepia, contraste, lumineux). Applique en ligne sur l'image, conserve dans l'export et le lien partage; les controles n'apparaissent pas sur la page publiee;
- commandes de mise en forme dans le chat : « fond sombre », « style magazine », « affiche », « police manuscrite », « accent vert », « premiere personne »… sont reconnues (FR/EN/中文) et appliquees automatiquement a la page;
- anti-robot Turnstile optionnel sur la publication : inactif tant que sitekey (`app.js`) et secret (`wrangler secret put TURNSTILE_SECRET`) ne sont pas configures; `/ai` reste protege par une limite de debit;
...
- module "Partager mon portail" apres generation avec publication, retour d'etat visible, adresse permanente et copie du lien;
