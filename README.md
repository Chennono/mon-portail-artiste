...
- lien partageable instantane: « Copier le lien partageable » encode toute la page (textes, style, mise en page, palette, polices) dans l'URL (`#p=...`). Ouvrir ce lien affiche la page seule, en lecture (sans editeur) — aucun hebergement requis, ideal pour les artistes peu a l'aise avec le numerique. Les photos importees ne sont pas incluses dans le lien (trop long); pour une page avec photos, utiliser « Exporter HTML » + hebergement;
- publication automatique optionnelle (« Publier ») : envoie la page (avec photos) a un petit service Cloudflare Worker qui la met en ligne et renvoie un vrai lien `https://.../p/<id>`. Le bouton n'apparait que si `PUBLISH_ENDPOINT` est renseigne dans `app.js`. Code et instructions de deploiement dans `worker/` (Worker + KV, pages expirant a 90 jours);
