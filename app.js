...
const DEFAULT_PROFILE_PHOTO_SETTINGS = { size: 85, side: "right", focus: "top", shape: "rectangle", frame: "none", filter: "none" };
...
function bindEvents() {
  ...
  // Les réglages de style de la photo (taille, forme, cadre…) sont dans la barre en aperçu.
  ...
  preview.innerHTML = `
    ...
    <header class="portal-hero${state.profilePhoto ? ` has-profile-photo photo-side-${state.profilePhotoSettings.side}` : ""}">
      ...
      <div class="portal-visual-wrap">
        ...
        ${heroImageControl()}
        ${state.profilePhoto ? profileStyleControls() : imageStyleControls("hero")}
      </div>
    </header>
...
function bindImageStyleControls() {
  ...
  $$("#portalPreview [data-photo-prop]").forEach((control) => {
    control.addEventListener("change", () => updateProfilePhotoSetting(control.dataset.photoProp, control.value));
  });
}
...
function renderHeroVisual() {
  ...
  if (state.profilePhoto) {
    const filter = IMG_STYLE_FILTER[state.profilePhotoSettings.filter] || "";
    const filterStyle = filter ? ` style="filter:${filter}"` : "";
    return `<img src="${state.profilePhoto.dataUrl}" alt="${escapeHtml(state.profilePhoto.alt || t("photo.defaultAlt"))}"${filterStyle}>`;
  }
...
function normalizeProfilePhotoSettings(value) {
  ...
  return {
    ...
    filter: Object.prototype.hasOwnProperty.call(IMG_STYLE_FILTER, source.filter) ? source.filter : DEFAULT_PROFILE_PHOTO_SETTINGS.filter
  };
}
...
function updateProfilePhotoSetting(prop, value) {
  state.profilePhotoSettings = normalizeProfilePhotoSettings({ ...state.profilePhotoSettings, [prop]: value });
  renderPreview();
  persist();
}

function profileStyleControls() {
  const s = state.profilePhotoSettings;
  const sel = (prop, labelKey, opts) =>
    `<label>${escapeHtml(t(labelKey))}<select data-photo-prop="${prop}">${opts
      .map(([value, key]) => `<option value="${value}"${s[prop] === value ? " selected" : ""}>${escapeHtml(t(key))}</option>`)
      .join("")}</select></label>`;
  return `<div class="image-style-bar preview-only">
    <label>${escapeHtml(t("photo.size"))}<input type="range" data-photo-prop="size" min="45" max="100" step="5" value="${s.size}"></label>
    ${sel("side", "photo.side", [["right", "photo.sideRight"], ["left", "photo.sideLeft"]])}
    ${sel("focus", "photo.focus", [["top", "photo.focusTop"], ["center", "photo.focusCenter"], ["bottom", "photo.focusBottom"]])}
    ${sel("shape", "photo.shape", [["rectangle", "photo.shapeRectangle"], ["rounded", "photo.shapeRounded"], ["circle", "photo.shapeCircle"], ["arch", "photo.shapeArch"]])}
    ${sel("frame", "photo.frame", [["none", "photo.frameNone"], ["thin", "photo.frameThin"], ["double", "photo.frameDouble"], ["shadow", "photo.frameShadow"]])}
    ${sel("filter", "img.filter", [["none", "img.filter.none"], ["bw", "img.filter.bw"], ["sepia", "img.filter.sepia"], ["contrast", "img.filter.contrast"], ["bright", "img.filter.bright"]])}
  </div>`;
}
